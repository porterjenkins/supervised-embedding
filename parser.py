...
class GraphParser(object):
    ...
    def graphParser(self, xml_file = "jinan_large.osm"):
        xml_file = self.dao.road_dir + "/" + xml_file
        ...
        bigNode = {}
        bigNodeLanes = {}
        bigThre = self.dist_gen((36.64828, 117.0078), (36.64731, 117.00782)) * 1.5
        ...
        with open(self.dao.road_dir + "/bignode_coors.txt", "w") as nf:
            nf.write(str(bigNodeList) + "\n")
        print("Num of big Node: {0}".format(len(bigNode)))

        # write number of lanes
        with open(self.dao.road_dir + "/bignode_lanes.txt", "w") as bnl:
        ...
        # clean big node
        with open(self.dao.road_dir + "/bignodes.txt", "w") as bnf:
            for node in bigNode.values():
                bnf.write(",".join([str(node[0][0]), str(node[0][1][0]), str(node[0][1][1])]) + "\n")

        segThre = self.dist_gen((36.6538, 117.0074), (36.6784, 117.0180)) * 0.9
        ccnt = 0
        with open(self.dao.road_dir + "/bigedges.txt","w") as bnef:
